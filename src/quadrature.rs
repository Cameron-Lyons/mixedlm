...
pub fn gauss_hermite_nodes_weights(n: usize) -> (Vec<f64>, Vec<f64>) {
    match n {
        ...
        3 => (
            vec![-1.2247448713915890, 0.0, 1.2247448713915890],
...
fn compute_gauss_hermite(n: usize) -> (Vec<f64>, Vec<f64>) {
    ...
    for i in 0..n {
        ...
        for j in 1..=n {
            let p3 = p2;
            p2 = p1;
            p1 = nodes[i] * (2.0 / j as f64).sqrt() * p2
                - ((j - 1) as f64 / j as f64).sqrt() * p3;
        }
...
pub fn adaptive_gauss_hermite_1d(
    ...
    let adapted_nodes: Vec<f64> = nodes
        .iter()
        .map(|&x| mode + sqrt2 * scale * x)
        .collect();
