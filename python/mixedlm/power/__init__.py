...
class PowerResult:
    ...
    def __str__(self) -> str:
        lines = [
            f"Power analysis by simulation",
            f"",
...
def powerSim(
    ...
    if test is None:
        ...
        if len(param_names) > 1:
            test_param = param_names[1]
        else:
            test_param = param_names[0]
        test_func: Callable[[LmerResult | GlmerResult], bool] = lambda m: _default_test(
            m, test_param, alpha
        )
    elif isinstance(test, str):
        test_param = test
        test_func = lambda m: _default_test(m, test_param, alpha)
    else:
        test_func = test
...
def powerCurve(
    ...
    for i, v in enumerate(values):
        ...
        if along == "effect_size":
            modified_model = _scale_effect(model, v, test if isinstance(test, str) else None)
            result = powerSim(
                modified_model, test=test, nsim=nsim, alpha=alpha, seed=iter_seed, verbose=False
            )
        else:
            result = powerSim(model, test=test, nsim=nsim, alpha=alpha, seed=iter_seed, verbose=False)
