...
class NlmerResult:
        ...
        **kwargs
        ...
        if newresp is None:
            ...
            if len(newresp) != len(self.y):
                raise ValueError(
                    f"newresp has length {len(newresp)}, expected {len(self.y)}"
                )
    ...
    def getME(self, name: str):
        ...
        if name == "phi":
            ...
            valid_names = [
                "phi", "theta", "sigma", "b", "y", "x", "groups",
                "n", "n_obs", "n_groups", "deviance", "weights", "offset",
                "group_levels", "random_params",
            ]
    ...
    def __init__(
        ...
        if weights is not None:
            ...
            if len(self.weights) != len(self.y):
                raise ValueError(
                    f"weights has length {len(self.weights)}, expected {len(self.y)}"
                )
        else:
            self.weights = None
        ...
        if offset is not None:
            ...
            if len(self.offset) != len(self.y):
                raise ValueError(
                    f"offset has length {len(self.offset)}, expected {len(self.y)}"
                )
